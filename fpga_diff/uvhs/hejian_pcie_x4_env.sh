...
export UVHS_CPU_CLK_PERIOD_NS="${UVHS_CPU_CLK_PERIOD_NS:-8}"
...
uvhs_hejian_pcie_check_env() {
  ...
  check_equal UVHS_CPU_CLK_PERIOD_NS 8
  ...
  check_grep "fpga_diff UVHS x4 target uses relaxed NutShell CPU clock" '^uvhs_hejian_pcie_x4_nutshell_all:[[:space:]]+UVHS_CPU_CLK_PERIOD_NS[[:space:]]*=[[:space:]]*8' "$_uvhs_hejian_pcie_root_dir/uvhs/Makefile"
