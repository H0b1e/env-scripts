...
proc print_help {} {
    puts "Usage:"
    puts "  vivado -mode batch -source script/export_vivado_ip.tcl -tclargs \\"
    puts "    --origin_dir <fpga_diff> --out_dir <fpga_diff> \[--vivado_version <ver>\] \[--cpu <name>\] \[--core_dir <dir>\] \[--only <ip>\] \[--skip <ip>\] \[--force\] \[--jobs N\]"
    exit 0
}
...
# IP creation scripts live next to this flow under script/ip (moved out of the
# legacy src/tcl/common tree).
set tcl_dir [file join $origin_dir script ip]
...
# Vivado 2024.2 can emit CDC waiver warnings from Xilinx-generated AXI
# Interconnect XDCs when auto clock/data converter contexts are optimized or
# expanded during OOC synthesis. These messages are generated constraint noise,
# not a design CDC finding, and otherwise block the clean-log gate.
set_msg_config -id {Vivado_Tcl 4-920} -suppress
set_msg_config -id {Vivado_Tcl 4-921} -suppress
...
proc run_and_write_opened_dcp {run_name out_file jobs force} {
    ...
    # Synthesize as an out-of-context module with no IO buffers, mirroring the
    # ln_quad reference flow (run_ln_quad_synth.tcl). These checkpoints are
    # instantiated as blackbox sub-modules inside the UVHS top; default
    # top-level synthesis would insert IBUF/OBUF on the boundary ports, which
    # is illegal mid-hierarchy.
    set_property -name {STEPS.SYNTH_DESIGN.ARGS.MORE OPTIONS} \
                 -value {-mode out_of_context -no_iobuf} -objects $run
    ...
    # Emit a synth stub next to the checkpoint so the UVHS assemble flow has a
    # blackbox module declaration to bind the DCP against (the DDR IP ships its
    # own Stub.v; the locally exported BDs need one too).
    set stub_file "[file rootname $out_file]_stub.v"
    write_verilog -mode synth_stub -force $stub_file
    puts "INFO: exported synth stub $stub_file"
}
...
proc export_bd_ip {name script out_file jobs force} {
    ...
    # Strip debug instrumentation so the exported OOC checkpoint carries no
    # ILA / debug-hub logic, mirroring run_ln_quad_synth.tcl. BD nets tagged
    # with HDL_ATTRIBUTE.DEBUG (e.g. AXI_bridge) would otherwise auto-insert
    # system_ila cores during validate/generate. No-op on BDs that mark
    # nothing for debug (xdma_ep, data_bridge, jtag_ddr_subsys).
    foreach net [get_bd_intf_nets -quiet] {
        catch {set_property HDL_ATTRIBUTE.DEBUG false $net}
    }
    foreach net [get_bd_nets -quiet] {
        catch {set_property HDL_ATTRIBUTE.DEBUG false $net}
    }
    foreach c [get_bd_cells -quiet -filter {(VLNV =~ "*:ila:*") || (VLNV =~ "*:system_ila:*")}] {
        catch {delete_bd_objs $c}
        puts "INFO: stripped debug cell $c from BD $name"
    }
